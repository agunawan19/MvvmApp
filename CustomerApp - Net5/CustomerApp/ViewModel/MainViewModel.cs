...
namespace CustomerApp.ViewModel
{
    public class MainViewModel : ObservableObject
    {
        ...
        {
            var collection = CollectionViewSource.GetDefaultView(Customers);
            collection.Filter = !string.IsNullOrWhiteSpace(textToSearch)
                ? (Predicate<object>) (c => ((Customer) c).Country.ToLower().Contains(textToSearch.ToLower()))
                : null;
        }
    }
}